...
export const updatePolicy = async (req, res, next) => {
  try {
    ...
    // Input Validation
    const errors = [];
    if (algorithm !== undefined && !['fixed-window', 'sliding-window', 'sliding-window-counter', 'token-bucket', 'leaky-bucket'].includes(algorithm)) {
      errors.push("Algorithm must be 'fixed-window', 'sliding-window', 'sliding-window-counter', 'token-bucket', or 'leaky-bucket'.");
    }
